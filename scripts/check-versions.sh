...
if [ -d "/Applications/Ghostty.app" ]; then
  ...
  if [ -f "$HOME/.config/ghostty/config" ]; then
    ...
    # Validate configuration using Ghostty's built-in validator
    print_status "Validating Ghostty configuration..."
    if command -v ghostty &>/dev/null; then
      if config_validation=$(ghostty +validate-config --config-file="$HOME/.config/ghostty/config" 2>&1); then
        print_success "Ghostty configuration is valid"
      else
        print_error "Ghostty configuration validation failed:"
        echo "$config_validation"
      fi
    else
      # Try to validate with the config file in the repository
      repo_config="./ghostty/config"
      if [ -f "$repo_config" ]; then
        print_status "Validating repository Ghostty configuration..."
        if config_validation=$(ghostty +validate-config --config-file="$repo_config" 2>&1); then
          print_success "Repository Ghostty configuration is valid"
        else
          print_error "Repository Ghostty configuration validation failed:"
          echo "$config_validation"
        fi
      fi
    fi
